// https://leetcode.com/problems/binary-tree-zigzag-level-order-traversal/submissions/1
#include <bits/stdc++.h>
using namespace std;
...
public:
    void insertToAnswer(vector<vector<int>>& answer, pair<TreeNode*, int> p){
        if(answer.size() - 1 == p.second){
            if(p.second % 2 == 0) answer.back().push_back(p.first->val);
            else answer.back().insert(answer.back().begin(), p.first->val);
        } else {
