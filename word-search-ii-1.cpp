...
public:
    vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
        ...
        for(auto word : words){
            ...
            ff(i, 0, m){
                ff(j, 0, n){
                    if (word[0] == board[i][j] && find(board, word, visited, 1, i, j, m, n))
                        ans[word] = true;
                }
            }
        }
    ...
    bool find(vvc& board, string& word, vvb& visited, int idx, int i, int j, int m, int n){
        ...
        for(auto step : steps){
            ...
            if (ii >= 0 && ii < m && jj >= 0 && jj < n && word[idx] == board[ii][jj])
                flag = flag || find(board, word, visited, idx + 1, ii, jj, m, n);
        }
